...
// ============================================================
// widgets/common_widgets.dart
// Reusable widgets — upgraded with Firebase image + error states
// ============================================================
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            // ── Thumbnail ──────────────────────────────
            _ItemThumbnail(item: item, isLost: isLost),
...
// ── Item Thumbnail (Firebase image or emoji fallback) ──────
class _ItemThumbnail extends StatelessWidget {
  final ItemModel item;
  final bool isLost;
  const _ItemThumbnail({required this.item, required this.isLost});

  @override
  Widget build(BuildContext context) {
    final bg = isLost
        ? const LinearGradient(colors: [Color(0xFFFFEBEE), Color(0xFFFFCDD2)])
    ...
    return Container(
      ...
      margin: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        gradient: bg, borderRadius: BorderRadius.circular(14),
      ),
      ...
      child: item.imageUrl != null && item.imageUrl!.isNotEmpty
          ? CachedNetworkImage(
              ...
              errorWidget: (_, __, ___) => Center(
                child: Text(item.emojiPlaceholder,
                    style: const TextStyle(fontSize: 36)),
              ),
            )
          : Center(
...
class _AIScoreBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = score >= 85
        ? Colors.green : score >= 75
        ? Colors.teal  : score >= 50
